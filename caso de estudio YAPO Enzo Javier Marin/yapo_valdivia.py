...
from urllib.request import urlopen as uReq # libreria para realizar consultas HTTP, envia consultas get que devuelve la pagina html
from bs4 import BeautifulSoup as soup # Libreria para leer un archivo html y extraer informacion
import pandas as pd
import xlsxwriter
import urllib.request
from selenium import webdriver # selenium es una libreria que permite simular una navegacion en un navegador web, permite definir que navegador web se va simular
import time

profile = pd.DataFrame(columns=['Link', 'Title', 'Description', 'Price','Date', 'Name', 'Category'])

browser = webdriver.Chrome() ## se simula el navegador Chrome
myUrl = 'https://www.yapo.cl/los_rios/todos_los_avisos?ca=11_s&l=0&w=1&cmn=243'
browser.get(myUrl)
pageSoup = soup(browser.page_source, 'html.parser')

pages = pageSoup.find('span',  {'class', 'nohistory FloatRight'}).a['href'] ## con a['href'] se recuperan los enlaces 
...
for i in range(lastPage): # navegar por todas las paginas de anuncios
    url = pages + str(i+1)
    browser.get(url) # obtiene la url de cada anuncio 
    pageSoup = soup(browser.page_source, 'html.parser')
    ...
    for link in links:
        ...
        if(pageSoup.find('div', {"class" : "description"})): ## recupera la seccion descripcion
            try:
...
## Estas ultimas lineas sirven para guardar los datos en un archivo
##


#print(profile)
        filename = 'fre.xlsx' # se guardan los datos en un archivo excel
